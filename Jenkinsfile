                ...
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                    sh '''
                        aws ssm send-command \
                          ...
                          --parameters commands=["docker pull ${ECR_REPO}:latest", "docker stop my-static-container || true", "docker rm my-static-container || true", "docker run -d -p 80:3000 --restart always --name my-static-container ${ECR_REPO}:latest"] \
                          --timeout-seconds 600 \
                          --region ${AWS_REGION}
                    '''
                }
